...
const BLOCK_ELEMENT_TAGS: ReadonlyArray<string> = [
    'article',
    'header',
    'aside',
    'hgroup',
...
const SELF_CLOSING_ELEMENT_TAGS: ReadonlyArray<string> = [
    'area',
    'base',
    'br',
    'col',
...
const TEXT_AST_TYPES: ReadonlyArray<string> = ['text', 'textNode'];
...
const getType: Function = Object.prototype.toString;
...
function extractDefinitionsFromASTTree(ast: ASTNode, parser: Function) {
...
function formExtraPropsForHTMLNodeType(props: object = {}, ast: ASTNode, definitions) {
    switch (ast.type) {
...
function getHTMLNodeTypeFromASTNodeType(node: ASTNode) {
    switch (node.type) {
    case 'break':
        return 'br';
...
function attributeValueToJSXPropValue(key: string, value: any) {
    if (key === 'style') {
...
function isCoalesceableHTML(html: string) {
...
function coalesceInlineHTML(ast: ASTNode) {
    function coalescer(node: ASTNode, index: number, siblings: ASTNode[]) {
        if (node.type === 'html') {
...
export function compiler(markdown: string, {overrides = {}} = {}) {
    let definitions;
    let footnotes;

    function astToJSX(ast: ASTNode, index: number = 0) { /* `this` is the dictionary of definitions */
        ...
        if (ast.type === 'html') {
            return (
                <div key={index} dangerouslySetInnerHTML={{__html: ast.value}} />
            );
        ...
        if (ast.type === 'table') {
            ...
            ast.children = ast.children.reduce((children, child, index: number) => {
                if (index === 0) {
        ...
        let props: any = {key: index, ...ast.props};
 ...
 */
const Component = ({children, options, ...props}: ComponentProps) => compiler(children, options);
